  ...
  def self.register(bot)
    bot.command('watch') do |ctx|
      ...
      if parts.nil? || parts.length < 2
        ctx.reply("🎬 *Watch a movie*\n\nUsage: `/watch <name> <code>`", parse_mode: "Markdown")
        next
      end
      ...
      unless valid_code?(code)
        ctx.reply("❌ Invalid code. Get a valid code from the group.", parse_mode: "Markdown")
        next
      end
      ...
      if video.nil?
        ctx.reply("❌ *#{name}* not found.\n\nTry `/search #{name}` first.", parse_mode: "Markdown")
        next
      end
